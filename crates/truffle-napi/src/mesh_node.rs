                ...
                match lifecycle_rx.recv().await {
                    Ok(event) => match event {
                        ...
                        ShimLifecycleEvent::Peers(peers_data) => {
                            ...
                            // Without this, devices see each other via Tailscale
                            // but can't exchange mesh protocol messages.
                            ...
                            if let Some(ref shim) = *shim_guard {
                                for peer in &peers_data.peers {
                                    ...
                                    // Only dial peers that look like truffle nodes
                                    // (hostnames generated by truffle contain the prefix)
                                    if !peer.hostname.contains(&hostname_prefix) {
                                        continue;
                                    }
                                    let dns = peer.dns_name.clone();
                                    tracing::info!("Dialing peer {dns} via sidecar");
                                    match shim.dial(dns.clone(), 443).await {
                                        Ok(_rx) => {
